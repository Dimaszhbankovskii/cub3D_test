...
# Makefile for PARSER ======>> need to delete
...
SRCS_LIST			=	main_parser.c

SRCS				=	$(addprefix $(SRCS_DIR)/,$(SRCS_LIST))

SRCS_LIST_PARSER	=	token.c

SRCS_PARSER			=	$(addprefix $(SRCS_DIR_PARSER)/,$(SRCS_LIST_PARSER))

SRCS 				+= $(SRCS_DIR_PARSER)

OBJS_DIR			=	objs_parser

OBJS				=	$(addprefix $(OBJS_DIR)/,$(SRCS_LIST:.c=.o))
OBJS				+=	$(addprefix $(OBJS_DIR)/,$(SRCS_LIST_PARSER:.c=.o))

all : $(OBJS_DIR) $(NAME_PARSER)

$(NAME_PARSER) : $(OBJS) $(LIBFT)
	@$(CC) $(CFLAGS) $^ -o $@
	@echo "$(GREEN)Project succesfully compiled$(NORMAL)"

$(OBJS_DIR)/%.o : $(SRCS_DIR)/%.c $(INCLUDES)/$(PARSER_H) Makefile
	@$(CC) $(CFLAGS) -c $< -o $@
	@echo "$(BLUE)Creating object file$(WHITE) --> $(notdir $@) --> $(GREEN)[Done]$(NORMAL)"

$(OBJS_DIR)/%.o : $(SRCS_DIR)/parser/%.c $(INCLUDES)/$(PARSER_H) Makefile
	@$(CC) $(CFLAGS) -c $< -o $@
	@echo "$(BLUE)Creating object file$(WHITE) --> $(notdir $@) --> $(GREEN)[Done]$(NORMAL)"
...
fclean : clean
	@make fclean -C ./libft
	@rm -rf $(NAME_PARSER)
	@echo "$(VIOLET)Removing perfoming file$(WHITE) --> $(NAME_PARSER) --> $(GREEN)[Done]$(NORMAL)"
